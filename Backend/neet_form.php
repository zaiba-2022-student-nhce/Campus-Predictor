...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submitMedical'])) {
    $submitted = true; // Set submitted flag to true
    
    // Validate NEET marks
    if (empty(trim($_POST["neet_marks"]))) {
        $neet_marks_err = "Please enter your NEET marks.";
    } else {
        $neet_marks = trim($_POST["neet_marks"]);
    }
    ...
    if (empty($neet_marks_err)) {
        ...
        // Prepare a select statement with category and marks filter
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "is", $param_neet_marks, $param_category);

            // Set parameters
            $param_neet_marks = $neet_marks;
...
<head>
    ...
    <title>NEET 2025 College Predictor</title>
...
<nav class="navbar sticky-top navbar-expand-sm navbar-light" style="background-color:#003366;">
    ...
    <div  class="section-title">
            <h4>NEET 2025 College Predictor <i class="fa fa-medkit" aria-hidden="true"></i>
            </h4>
        </div>
        <div class="form-container">
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                <input class="ip" type="text" name="neet_marks" placeholder="NEET Marks" value="<?php echo htmlspecialchars($neet_marks); ?>" required>
                <span class="help-block"><?php echo $neet_marks_err; ?></span>
                
                <select class="ip" name="category">
                ...
                <input class="button" type="submit" name="submitMedical" value="SUBMIT">
            </form>
        </div>
    ...
    <?php
    // Displaying table of colleges based on NEET marks and category if form submitted
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submitMedical'])) {
        if ($colleges_found) {
            ...
            echo '<p>No colleges found matching the NEET marks and category criteria.</p>';
        }
    }
    ?>
