...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submitEngineering'])) {

    // Validate KCET rank
    if (empty(trim($_POST["kcet_rank"]))) {
        $kcet_rank_err = "Please enter your KCET rank.";
    } else {
        $kcet_rank = trim($_POST["kcet_rank"]);
    }
    ...
    if (empty($kcet_rank_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "is", $param_kcet_rank, $param_category);

            // Set parameters
            $param_kcet_rank = $kcet_rank;
...
<nav class="navbar sticky-top navbar-expand-sm navbar-light" style="background-color:#003366;">
    ...
    <div class="background"><div  class="section-title">
        <h4>KCET 2025 College Predictor <i class="fa fa-university" aria-hidden="true"></i></h4>
        </div>
        
        <div class="form-container">
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                <input class="ip" type="text" name="kcet_rank" placeholder="KCET Ranking" value="<?php echo htmlspecialchars($kcet_rank); ?>" required>
                <span class="help-block"><?php echo $kcet_rank_err; ?></span>
                ...
                <br><br>
                <input class="button" type="submit" name="submitEngineering" value="SUBMIT">
            </form>
        </div>
    ...
    <?php
    // Displaying table of colleges based on KCET rank and category if form submitted
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submitEngineering'])) {
        if ($colleges_found) {
            ...
            echo '<p>No colleges found matching the KCET rank and category criteria.</p>';
        }
    }
